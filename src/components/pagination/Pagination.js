        ...
        <div className={`pagination ${theme}-theme`}>
            {itemsList.length ? itemsList.length ? typeof itemsNumberInPage == "number" ? itemsList.length > itemsNumberInPage ? 
                <div className='pagination-list'>
                    <span className={`pagination-item before ${page == 1 ? 'disable' : ''}`} onClick={()=> setPage(page > 1 ? page-1 : page)}><MdOutlineArrowBackIosNew /></span>
                    ...
                    <span className={`pagination-item after ${page == Math.ceil(itemsList.length / itemsNumberInPage) ? 'disable' : ''}`} onClick={()=> setPage(page < Math.ceil(itemsList.length / itemsNumberInPage) ? page+1 : page)}><MdOutlineArrowForwardIos /></span>
                </div>
